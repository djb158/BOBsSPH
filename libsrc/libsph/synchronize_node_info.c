...
/*                                                                           */
  for (i_rank=0;i_rank<rank;i_rank++)
  {
    ...
    if ( (MPI_Recv(node_info[i_rank].burn_A,NPARTICLES,MPI_INT,i_rank,tag37,MPI_COMM_WORLD,&mpi_status)) !=MPI_SUCCESS)
    {
      printf(" SynchroniseNodeInfo(): unable to receive packet \n");
      exit_status = EXIT_FAILURE;
      goto RETURN;
  ...
  for (i_rank=0;i_rank<=cluster_size;i_rank++)
  {
    if (i_rank!=rank)
    {
      ...
      if ( (MPI_Send(node_info[rank].burn_A,NPARTICLES,MPI_INT,i_rank,tag37,MPI_COMM_WORLD)) != MPI_SUCCESS)
      {
        printf(" SynchroniseNodeInfo(): unable to send packet \n");
        exit_status = EXIT_FAILURE;
        goto RETURN;
...
/*                                                                           */
  for (i_rank=0;i_rank<rank;i_rank++)
  {
    ...
    if ( (MPI_Recv(node_info[i_rank].burn_B,NPARTICLES,MPI_INT,i_rank,tag38,MPI_COMM_WORLD,&mpi_status)) !=MPI_SUCCESS)
    {
      printf(" SynchroniseNodeInfo(): unable to receive packet \n");
      exit_status = EXIT_FAILURE;
      goto RETURN;
  ...
  for (i_rank=0;i_rank<=cluster_size;i_rank++)
  {
    if (i_rank!=rank)
    {
      ...
      if ( (MPI_Send(node_info[rank].burn_B,NPARTICLES,MPI_INT,i_rank,tag38,MPI_COMM_WORLD)) != MPI_SUCCESS)
      {
        printf(" SynchroniseNodeInfo(): unable to send packet \n");
        exit_status = EXIT_FAILURE;
        goto RETURN;
...
/*                                                                           */
  for (i_rank=0;i_rank<rank;i_rank++)
  {
    ...
    if ( (MPI_Recv(node_info[i_rank].burn_E,NPARTICLES,MPI_INT,i_rank,tag39,MPI_COMM_WORLD,&mpi_status)) !=MPI_SUCCESS)
    {
      printf(" SynchroniseNodeInfo(): unable to receive packet \n");
      exit_status = EXIT_FAILURE;
      goto RETURN;
  ...
  for (i_rank=0;i_rank<=cluster_size;i_rank++)
  {
    if (i_rank!=rank)
    {
      ...
      if ( (MPI_Send(node_info[rank].burn_E,NPARTICLES,MPI_INT,i_rank,tag39,MPI_COMM_WORLD)) != MPI_SUCCESS)
      {
        printf(" SynchroniseNodeInfo(): unable to send packet \n");
        exit_status = EXIT_FAILURE;
        goto RETURN;
...
/*                                                                           */
  for (i_rank=0;i_rank<rank;i_rank++)
  {
    ...
    if ( (MPI_Recv(node_info[i_rank].burn_F,NPARTICLES,MPI_INT,i_rank,tag40,MPI_COMM_WORLD,&mpi_status)) !=MPI_SUCCESS)
    {
      printf(" SynchroniseNodeInfo(): unable to receive packet \n");
      exit_status = EXIT_FAILURE;
      goto RETURN;
  ...
  for (i_rank=0;i_rank<=cluster_size;i_rank++)
  {
    if (i_rank!=rank)
    {
      ...
      if ( (MPI_Send(node_info[rank].burn_F,NPARTICLES,MPI_INT,i_rank,tag40,MPI_COMM_WORLD)) != MPI_SUCCESS)
      {
        printf(" SynchroniseNodeInfo(): unable to send packet \n");
        exit_status = EXIT_FAILURE;
        goto RETURN;
