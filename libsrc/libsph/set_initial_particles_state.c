...
#include "set_initial_particles_state.h"
...
PARTICLES *SetInitialParticlesState(PARS *pars,int particles_num,char *rank_name)
{
  ...
  char   fname[]                       = "...SetInitialParticlesState";
...
/*                                                                            */
  ...
  for(l=0;l<nparticles;l++)
  {
    ...
    if (x<X0-EPSILON_DOUBLE) 
    {
      node_info[rank].ghost[l]   = l;
      node_info[rank].ghost_x[l] = 1;
      node_info[rank].ghost_z[l] = 0;
      dist = fabs(X0-x);
      found = 0;
      for (j=0;j<nparticles;j++)
      {
        ...
        {
          node_info[rank].ghost_mirror[l] = node_info[rank].raw_index[j];
          found = 1;
          break;
        }
      }
      if (found == 0)
      {
        printf("1 Rank: %i l = %i mirror not found\n",rank,l);
      ...
      node_info[rank].species[l] = 1025;
    }
    if (x>X1+EPSILON_DOUBLE) 
    {
      node_info[rank].ghost[l]   =  l;
      node_info[rank].ghost_x[l] = -1;
      node_info[rank].ghost_z[l] =  0;
      ...
      for (j=0;j<nparticles;j++)
      {
        x1 = particles[0].x[0][j];
        y1 = particles[0].x[1][j];
        z1 = particles[0].x[2][j];
        if ( (fabs(X1-x1-dist)<EPSILON_DOUBLE) && (fabs(y1-y)<EPSILON_DOUBLE) &&  (fabs(z1-z)<EPSILON_DOUBLE) )
        {
          node_info[rank].ghost_mirror[l] = node_info[rank].raw_index[j];
          found = 1;
          break;
      ...
      if (found == 0)
      {
        printf("2 Rank: %i l = %i mirror not found\n",rank,l);
        if (FreeParticles(&particles,pars,NPARTICLES) == EXIT_FAILURE)
        {
          printf("Rank: %i %s():particles -error return from FreeParticles()\n",rank,fname);
          exit_status = EXIT_FAILURE;
      ...
      node_info[rank].species[l] = 1026;
    }

    if ( (z<Z0-EPSILON_DOUBLE) && (x >= X0-EPSILON_DOUBLE) && (x <=X1+EPSILON_DOUBLE) )
    {
      node_info[rank].ghost[l]   = l;
      node_info[rank].ghost_x[l] = 0;
      node_info[rank].ghost_z[l] = 1;
      dist = fabs(Z0-z);
      found = 0;
      for (j=0;j<nparticles;j++)
      {
        ...
        y0 = particles[0].x[1][j];
        z0 = particles[0].x[2][j];
        if ( (fabs(z0-Z0-dist)<EPSILON_DOUBLE) && (fabs(y0-y)<EPSILON_DOUBLE) &&  (fabs(x0-x)<EPSILON_DOUBLE) )
        {
          node_info[rank].ghost_mirror[l] = node_info[rank].raw_index[j];
          found = 1;
          break;
      ...
      if (found == 0)
      {
        printf("3 Rank: %i l = %i mirror not found\n",rank,l);
        if (FreeParticles(&particles,pars,NPARTICLES) == EXIT_FAILURE)
        {
          printf("Rank: %i %s():particles -error return from FreeParticles()\n",rank,fname);
          exit_status = EXIT_FAILURE;
...
/*
        ...
        exit_status = EXIT_FAILURE;
        if (particles == NULL)printf("2 S  is null \n");
        if (particles != NULL)printf("2 S is not null \n");
        exit(1);
*/
/* djb end*/

    if ( (z>Z1+EPSILON_DOUBLE) && (x >= X0-EPSILON_DOUBLE) && (x <=X1+EPSILON_DOUBLE) )
    {
      node_info[rank].ghost[l]   = l;
      node_info[rank].ghost_x[l] = 0;
      node_info[rank].ghost_z[l] = -1;
      ...
      for (j=0;j<nparticles;j++)
      {
        ...
        y1 = particles[0].x[1][j];
        z1 = particles[0].x[2][j];
        if ( (fabs(Z1-z1-dist)<EPSILON_DOUBLE) && (fabs(y1-y)<EPSILON_DOUBLE) &&  (fabs(x1-x)<EPSILON_DOUBLE) )
        {
          node_info[rank].ghost_mirror[l] = node_info[rank].raw_index[j];
          found = 1;
          break;
      ...
      if (found == 0)
      {
        printf("4 Rank: %i l = %i mirror not found\n",rank,l);
        if (FreeParticles(&particles,pars,NPARTICLES) == EXIT_FAILURE)
        {
          printf("Rank: %i %s():particles -error return from FreeParticles()\n",rank,fname);
          exit_status = EXIT_FAILURE;
      ...
      node_info[rank].species[l] = 1028;
    }

    if ( 
         ( (fabs(x-X0)<EPSILON_DOUBLE) 
                      &&
         ( (z<=Z1+EPSILON_DOUBLE) && (z>=Z0-EPSILON_DOUBLE)) )
                      ||
         ...
         ( (fabs(z-Z1)<EPSILON_DOUBLE) 
                      &&
         ( (x<=X1+EPSILON_DOUBLE) && (x>=X0-EPSILON_DOUBLE)) )
                      ||
       ...
       )
    {
      ...
      if (fabs(x-X0)<EPSILON_DOUBLE)
      {
        node_info[rank].species[l] = 2049;
        if ( (fabs(z-Z0)<EPSILON_DOUBLE) )
        {
          node_info[rank].species[l] = 2053;  /* <== this may be error */
        }
        else if ( (fabs(z-Z1)<EPSILON_DOUBLE) ) 
        {
          node_info[rank].species[l] = 2054;
        }
      }

      if (fabs(x-X1)<EPSILON_DOUBLE)
      {
        node_info[rank].species[l] = 2050;
        if ( (fabs(z-Z0)<EPSILON_DOUBLE) )
        {
          node_info[rank].species[l] = 2055;
        }
        else if ( (fabs(z-Z1)<EPSILON_DOUBLE) ) 
        {
          node_info[rank].species[l] = 2056;
        }
      }
      if (fabs(z-Z0)<EPSILON_DOUBLE)
      {
        node_info[rank].species[l] = 2051;
        if ( (fabs(x-X0)<EPSILON_DOUBLE) )
        {
          node_info[rank].species[l] = 2053;
        }
        else if ( (fabs(x-X1)<EPSILON_DOUBLE) ) 
        {
          node_info[rank].species[l] = 2055;
        }
      }
      if (fabs(z-Z1)<EPSILON_DOUBLE)
      {
        node_info[rank].species[l] = 2052;
        if ( (fabs(x-X0)<EPSILON_DOUBLE) )
        {
          node_info[rank].species[l] = 2054;
        }
  ...
  for(l=0;l<nparticles;l++)
  {
    if ( node_info[rank].species[l] < 0 )
    {
      printf(" Rank: %i l = %i species not set \n",rank,l);
    }
  }
  ...
  node_info[rank].n_ghost                 = LARGE_NEGATIVE_INT; //defunct
