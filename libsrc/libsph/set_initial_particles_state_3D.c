...
#include "set_initial_particles_state_3D.h"
...
PARTICLES *SetInitialParticlesState3D(PARS *pars,int particles_num,char *rank_name)
{
  ...
  char   fname[]                       = "...SetInitialParticlesState3D";
...
/*                                                                            */
  ...
  Y   = (double *)calloc(PARTICLES_IN_Y,sizeof(double));
  Z   = (double *)calloc(PARTICLES_IN_Z,sizeof(double));
  dz  = (Z1-Z0)/(double)(PARTICLES_IN_Z-2*ZERO-1);
  dy  = (Y1-Y0)/(double)(PARTICLES_IN_Y-2*ZERO-1);
  ...
  for (k=0;k<PARTICLES_IN_Y;k++)
  {
    y = Y0*(1.0 - (double)(k-ZERO)*dy );
      ...
      if (i <= CENTRE_POINT)
      {
        ...
        if (i<CENTRE_POINT-20)
        {
          for (j=-ZERO;j<PARTICLES_IN_Z-ZERO;j++)
          {
            ...
            raw_particle[0].x[1][raw_index]       = y;
...
/*                                                                           */
/*                 set species for interior particles                        */
/*                                                                           */
  for(l=0;l<nparticles;l++)
  {
    ...
    if (
          ((ids=SetParticleID_3D(pars,particles,x,y,z)) == NULL)
                            ||
          (ids->species == EXIT_FAILURE)
       )
    {
      printf("Rank: %i %s():particles -error return from SetParticleID_3D() -species = %i\n",rank,fname,ids->species);
