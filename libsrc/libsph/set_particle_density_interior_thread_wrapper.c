...
#include "set_particle_density_interior_thread_wrapper.h"

int SetParticleDensity_INTERIOR_thread_wrapper(PARTICLES *particles,PARTICLES *x_particles,PARS *pars)
{
  ...
  char   fname[]                       = "...SetParticleDensity_INTERIOR_thread_wrapper";
...
#include "pars_include.h"
  ...
  for (i=0; i<COMPUTE_THREADS; i++)
  {
    err[i] = pthread_create(&thread[i], NULL,SetParticleDensity_INTERIOR, data+i);
  }
  ...
  for (i=0; i<COMPUTE_THREADS; i++)
  {
    if ((*data[i].exit_status == EXIT_FAILURE) || (err[i] != 0) )
    {
      ...
      printf(" Rank: %i  %s() -error return from pthread_create for SetParticleDensity_INTERIOR() \n",rank,fname);
      goto RETURN;
    }
  }
