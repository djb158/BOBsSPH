...
#include "set_initial_particles_state_1D.h"
...
/*                                                                            */
  ...
  for(l=0;l<nparticles;l++)
  {
    ...
    if (
          ((ids=SetParticleID_1D(pars,particles,x,y,z)) == NULL)
                            ||
          (ids->species == EXIT_FAILURE)
       )
    {
      printf("Rank: %i %s():particles -error return from SetParticleID_1D() -species = %i\n",rank,fname,ids->species);
