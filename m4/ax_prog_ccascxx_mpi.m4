...
# SYNOPSIS
#
#   AX_PROG_CCASCXX_MPI([MPI-WANTED-TEST[, ACTION-IF-FOUND[, ACTION-IF-NOT-FOUND]]])
#
# DESCRIPTION
#
...
#   standard variable CC with the found c++ compiler.
#
...
#     ],,[with_mpi=auto])
#
#     AX_PROG_CCASCXX_MPI([test x"$with_mpi" != xno],[use_mpi=yes],[
#       use_mpi=no
...
AC_DEFUN([AX_PROG_CCASCXX_MPI], [
AC_PREREQ(2.50)

# Check for compiler
# Needs to be split off into an extra macro to ensure right expansion
# order.
AC_REQUIRE([_AX_PROG_CCASCXX_MPI],[_AX_PROG_CCASCXX_MPI([$1])])

AS_IF([test x"$_ax_prog_ccascxx_mpi_mpi_wanted" = xno],
  [ _ax_prog_ccascxx_mpi_mpi_found=no ],
  [
    AC_LANG_PUSH([C++])
...
using MPI::Init;],[MPI::Init;])],
        [ _ax_prog_ccascxx_mpi_mpi_found=yes ],
        [ _ax_prog_ccascxx_mpi_mpi_found=no ])
      AC_MSG_RESULT($_ax_prog_ccascxx_mpi_mpi_found)
      if test "x$_ax_prog_ccascxx_mpi_mpi_found" = "xyes"; then
        break;
      fi
      LIBS=$save_LIBS
    done
    ...
    # Check for header
    AS_IF([test x"$_ax_prog_ccascxx_mpi_mpi_found" = xyes], [
...
])dnl AX_PROG_CCASCXX_MPI

dnl _AX_PROG_CCASCXX_MPI is an internal macro required by AX_PROG_CCASCXX_MPI.
dnl To ensure the right expansion order, the main function AX_PROG_CCASCXX_MPI
...
dnl  CC
dnl
AC_DEFUN([_AX_PROG_CCASCXX_MPI], [
  AC_ARG_VAR(MPICXX,[MPI C++ compiler command])
  ifelse([$1],,[_ax_prog_ccascxx_mpi_mpi_wanted=yes],[
    AC_MSG_CHECKING([whether to compile using MPI])
    if $1; then
       echo "HERE 0"
      _ax_prog_ccascxx_mpi_mpi_wanted=yes
    else
       echo "HERE 1"
      _ax_prog_ccascxx_mpi_mpi_wanted=no
    fi
    AC_MSG_RESULT($_ax_prog_ccascxx_mpi_mpi_wanted)
  ])
    echo "HERE 2"
    if test x"$_ax_prog_ccascxx_mpi_mpi_wanted" = xyes; then
    echo "HERE 3"
    ...
    echo "HERE 5"
        AC_CHECK_TOOLS([CC], [mpic++ mpicxx mpiCC sxmpic++ hcp mpxlC_r mpxlC mpixlcxx_r mpixlcxx mpg++ mpc++ mpCC cmpic++ mpiFCC CCicpc pgCC pathCC sxc++ xlC_r xlC bgxlC_r bgxlC openCC sunCC crayCC g++ c++ gpp aCC CC cxx cc++ cl.exe FCC KCC RCC])
      fi
    fi
  AC_PROG_CXX
